...
public final class CharacterNatures {
	...
	static {
		for (int i = 1; i < Character.MAX_VALUE; i++) {
			if (i >= '0' && i <= '9') {
				NATURES[i] = BitUtils.set(NATURES[i], NUMBER);
			} else if ((i >= 'a' && i <= 'z') || (i >= 'A' && i <= 'Z')) {
				NATURES[i] = BitUtils.set(NATURES[i], LETTER);
			} else if (i == '\b' || i == '\t' || i == '\n' || i == '\r' || i == ' ' || i == 160) {
				NATURES[i] = BitUtils.set(NATURES[i], SPACE);
				NATURES[i] = BitUtils.set(NATURES[i], SYMBOLS_AND_PUNCTUATION);
			}
			...
			if (ub == Character.UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS
				...
				NATURES[i] = BitUtils.set(NATURES[i], CJK);
				NATURES[i] = BitUtils.set(NATURES[i], CJK_CHINESE);
			} else if (ub == Character.UnicodeBlock.HALFWIDTH_AND_FULLWIDTH_FORMS
					|| ub == Character.UnicodeBlock.HANGUL_SYLLABLES || ub == Character.UnicodeBlock.HANGUL_JAMO
					|| ub == Character.UnicodeBlock.HANGUL_COMPATIBILITY_JAMO || ub == Character.UnicodeBlock.HIRAGANA
					|| ub == Character.UnicodeBlock.KATAKANA
					|| ub == Character.UnicodeBlock.KATAKANA_PHONETIC_EXTENSIONS) {
				NATURES[i] = BitUtils.set(NATURES[i], CJK);
				NATURES[i] = BitUtils.set(NATURES[i], CJK_NONCHINESE);
			}
		...
		for (char c : "！@#￥%……&×（），。；‘’“”：【】『』、《》？·～,./;':\"\\[]{}`~!@#$%^&*()_+-=|".toCharArray()) {
			NATURES[c] = BitUtils.set(NATURES[c], SYMBOLS_AND_PUNCTUATION);
		}

		for (char c : "（【『《[{(".toCharArray()) {
			NATURES[c] = BitUtils.set(NATURES[c], BRACKETS_HEAD);
		}
		for (char c : "）】』》]})".toCharArray()) {
			NATURES[c] = BitUtils.set(NATURES[c], BRACKETS_TAIL);
		}
		for (char c : "一二两三四五六七八九十零壹贰叁肆伍陆柒捌玖拾百千万亿拾佰仟萬億兆卅廿".toCharArray()) {
			NATURES[c] = BitUtils.set(NATURES[c], CJK_CHINESE_NUMBER);
		}

		for (char c : new char[] { '#', '&', '+', '-', '.', '@', '_' }) {
			NATURES[c] = BitUtils.set(NATURES[c], LETTER_CONNECTOR);
		}

		for (char c : new char[] { ',', '.' }) {
			NATURES[c] = BitUtils.set(NATURES[c], NUMBER_CONNECTOR);
		}

		for (char c : new char[] { '?', '*', '+' }) {
			NATURES[c] = BitUtils.set(NATURES[c], WILDCARD);
		}
	}
	...
	public static boolean is(char c, int type) {
		return BitUtils.is(NATURES[c], type);
	}
